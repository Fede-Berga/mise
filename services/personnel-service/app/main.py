"""FastAPI application factory for the personnel-service."""
...
from app.api.controllers.personnel_controller import router as personnel_router
...
def create_app() -> FastAPI:
    """Construct the personnel-service FastAPI application."""
    app = FastAPI(
        title="Mise Personnel Service",
        description="Manages staff members, roles, and scheduling for restaurant operations.",
        version="0.1.0",
        lifespan=lifespan,
    )
    register_middleware(app)
    app.include_router(build_health_router(service_name="personnel-service"))
    app.include_router(
        personnel_router,
        prefix="",
        tags=["personnel"],
