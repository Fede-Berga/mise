"""FastAPI application factory for the finance-service."""
...
from app.api.controllers.finance_controller import router as finance_router
...
def create_app() -> FastAPI:
    """Construct the finance-service FastAPI application."""
    app = FastAPI(
        title="Mise Finance Service",
        description="Records financial transactions and provides daily P&L summaries.",
        version="0.1.0",
        lifespan=lifespan,
    )
    register_middleware(app)
    app.include_router(build_health_router(service_name="finance-service"))
    app.include_router(
        finance_router,
        prefix="/finance",
        tags=["finance"],
